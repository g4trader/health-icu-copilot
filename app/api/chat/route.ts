...
function handleAgentOpinionIntent(patientId: string, agentId: ClinicalAgentId): { 
  ...
  if (mockOpinion) {
    ...
    const footer = '\n\n---\n*Iatron v1.0.0* | Dados: Simulados | Parecer de agente de subespecialidade. Este conteúdo é apenas apoio à decisão e não substitui avaliação médica presencial.';
...
function handleFallbackIntent(): { reply: string; showIcuPanel: boolean } {
  return {
    reply: (
      "Olá! Sou o **Iatron**, um assistente de IA para apoio à decisão clínica.\n\n" +
      "Posso ajudar com:\n\n" +
...
export async function POST(req: Request) {
  ...
  const focusedId = body.focusedPatientId ?? body.patientId ?? null;
    ...
    const transparencyFooter = `\n\n---\n**Iatron v${VERSION}** | Dados: Simulados | ${llmUtilizado ? `LLM: ${MODEL_VERSION}` : "Processamento determinístico"}`;
